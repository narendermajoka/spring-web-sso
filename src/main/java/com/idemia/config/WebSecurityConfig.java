...
import com.idemia.core.SAMLUserDetailsServiceImpl;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter implements InitializingBean, DisposableBean {
	...
	public KeyManager keyManager() {
		...
		try {
			keystore = keystoreFactory.loadKeystore("classpath:/localhost.cert", "classpath:/localhost.key.der",
					"localhost", "", null);
			passwords = new HashMap<String, String>();
			passwords.put("localhost", "");
		} catch (Exception e) {
			e.printStackTrace();
		}

		return new JKSKeyManager(keystore, passwords, "localhost");
	}
	...
	public ExtendedMetadataDelegate ssoCircleExtendedMetadataProvider() throws MetadataProviderException {
		 ...
		 * idpSSOCircleMetadataURL); httpMetadataProvider.setParserPool(parserPool());
		 */
		FilesystemMetadataProvider fileMetadataProvider;
		try {
			fileMetadataProvider = new FilesystemMetadataProvider(
					new ClassPathResource("/ssocircle_metadata.xml").getFile());
		} catch (IOException e) {
			e.printStackTrace();
		...
		ExtendedMetadataDelegate extendedMetadataDelegate = new ExtendedMetadataDelegate(fileMetadataProvider,
				extendedMetadata());
	...
	public CachingMetadataManager metadata() throws MetadataProviderException {
		...
		providers.add(oktaMetaDataProvider());
		return new CachingMetadataManager(providers);
	}

	@Bean
	@Qualifier("idp-okta")
	public ExtendedMetadataDelegate oktaMetaDataProvider() throws MetadataProviderException {
		FilesystemMetadataProvider fileMetadataProvider;
		try {
			fileMetadataProvider = new FilesystemMetadataProvider(
					new ClassPathResource("/okta_metadata.xml").getFile());
		} catch (IOException e) {
			e.printStackTrace();
			throw new MetadataProviderException(e.getMessage());
		}
	...
	public MetadataGenerator metadataGenerator() {
		...
		metadataGenerator.setEntityId("niravworks:spring:saml:sp");
	...
	// global logout
	/*
	 * @Bean public SAMLLogoutProcessingFilter samlLogoutProcessingFilter() { return
	...
	// messages
	/*
	 * @Bean public SAMLLogoutFilter samlLogoutFilter() { return new
	 * SAMLLogoutFilter(successLogoutHandler(), new LogoutHandler[] {
	 * logoutHandler() }, new LogoutHandler[] { logoutHandler() }); }
	 */
	@Bean
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/").permitAll().antMatchers("/saml/**").permitAll().antMatchers("/css/**")
				.permitAll().antMatchers("/img/**").permitAll().antMatchers("/js/**").permitAll().anyRequest()
				.authenticated();
		http.logout().deleteCookies("JSESSIONID").invalidateHttpSession(true).logoutSuccessUrl("/").permitAll()
